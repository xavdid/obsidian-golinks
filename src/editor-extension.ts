 ...
 */
export function createGoLinkDecoration(
  from: number,
  to: number,
  goLink: string
...
export function isInsideLink(view: EditorView, pos: number): boolean {
  try {
    const tree = syntaxTree(view.state);
    const node = tree.resolve(pos, 1);
    return node.type.name.includes("link") || node.type.name.includes("url");
  } catch (error) {
 ...
 */
export function findGoLinksInText(
  view: EditorView,
  from: number,
  to: number
        ...
        for (const { from, to } of view.visibleRanges) {
          ...
          if (isLivePreview) {
            ...
            for (const link of links) {
              ...
              const isInRenderedElement =
                lineText.trim().startsWith(">") || // blockquote
              ...
              if (!isInRenderedElement) {
                decorations.push(
                  createGoLinkDecoration(link.start, link.end, link.goLink)
                );
              }
            }
          } else {
